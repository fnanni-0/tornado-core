...
const Tornado = artifacts.require('./HumanTornado.sol')
const POH = artifacts.require('./Mocks/ProofOfHumanityMock.sol')
...
contract('HumanTornado', (accounts) => {
  let tornado
  let poh
  const sender = accounts[0]
  const operator = accounts[0]
  const levels = MERKLE_TREE_HEIGHT || 16
  const value = '0' // 0 ether
  let snapshotId
  let tree
  const fee = bigInt('0') // 0
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const pohAddress = await tornado.poh()
      pohAddress.should.be.equal(poh.address)
    })
  })

  describe('#deposit', () => {
    it('should throw if the user is not registered in Proof of Humanity', async () => {
      const commitment = toFixedHex(42)
      const error = await tornado.deposit(commitment, { from: sender }).should.be.rejected
      error.reason.should.be.equal('Not registered in Proof of Humanity')
    })

    it('should throw if the user has already enrolled', async () => {
      await poh.setHuman(sender, true)

      let commitment = toFixedHex(42)
      await tornado.deposit(commitment, { from: sender })

      let error = await tornado.deposit(commitment, { from: sender }).should.be.rejected
      error.reason.should.be.equal('The commitment has been submitted')

      commitment = toFixedHex(12)
      error = await tornado.deposit(commitment, { from: sender }).should.be.rejected
      error.reason.should.be.equal('Already enrolled in this batch')
    })
    ...
    it('should emit event', async () => {
      ...
      commitment = toFixedHex(12)
      await poh.setHuman(accounts[2], true)
      ;({ logs } = await tornado.deposit(commitment, { from: accounts[2] }))
...
// TODO: test after anonimization period starts.
if (false) {
  describe('#withdraw', () => {
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      tree.insert(deposit.commitment)
      await tornado.deposit(toFixedHex(deposit.commitment), { from: sender })
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await tornado.deposit(toFixedHex(deposit1.commitment), { gasPrice: '0' })
      await tornado.deposit(toFixedHex(deposit2.commitment), { gasPrice: '0' })
